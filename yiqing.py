...
def main():
    ...
    path_area = f'/users/mac/python/txt/yiqing/[{date.today()}]yiqing_area.json'
    creatfile(path_area, area)
    
    print('area文件已储存')

    country = parse_country(r.text)
    path_country = f'/users/mac/python/txt/yiqing/[{date.today()}]yiqing_country.json'
    creatfile(path_country, country)
    print('country文件已储存')

    r = request_handle(url[1])

    path_full = f'/users/mac/python/txt/yiqing/[{date.today()}]yiqing_full.json'
